...
const toolFunctionHost = document.getElementById("toolFunctionHost");
...
const toolDefinitions = {
  "Event Block Calculator": {
    shortName: "Event Block",
    status: "Live",
    render: renderEventBlock
  },
  "IronClock": {
    shortName: "IronClock",
    status: "Live",
    render: renderIronClock
  },
  "Gym Share": {
    shortName: "Gym Share",
    status: "Live",
    render: renderGymShare
  },
  "Session Log": {
    shortName: "Session Log",
    status: "Live",
    render: renderSessionLog
  },
  "Meet Planner": {
    shortName: "Meet Planner",
    status: "Live",
    render: renderMeetPlanner
  },
  "Load Sheet": {
    shortName: "Load Sheet",
    status: "Live",
    render: renderLoadSheet
  }
};
...
function writeHost(html) {
  if (toolFunctionHost) {
    toolFunctionHost.innerHTML = html;
  }
}
...
function selectTool(card) {
  ...
  const name = card.dataset.toolName || "Event Block Calculator";
  const definition = toolDefinitions[name] || toolDefinitions["Event Block Calculator"];
  const shortName = card.dataset.toolShort || definition.shortName;
  const status = definition.status || card.dataset.toolStatus || "Live";
...
const initialCard = document.querySelector("[data-tool-card].is-selected") || toolCards[0];
if (initialCard) {
